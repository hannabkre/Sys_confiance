...
import n7.petrinet.*;
import n7.petrinet.util.petrinetSwitch;

/**
 * Cette classe effectue la validation d'un modèle de réseau de Pétri. Elle
 * utilise le mécanisme de visite (Switch) généré par EMF pour vérifier que les
 * objets du modèle respectent les contraintes structurelles et sémantiques
 * définies par les exigences du projet (R2.1 à R2.6).
 * 
 * @author -
 */
public class PetrinetValidator extends petrinetSwitch<Boolean> {
	...
	/**
	 * Résultat de la validation (état interne réinitialisé à chaque nouvelle
	 * validation).
	 */
	...
	public Boolean casePetrinet(Petrinet net) {
		...
		this.result.recordIfFailed(net.getName() != null && net.getName().matches(IDENT_REGEX), net,
	...
	public Boolean casePlace(Place p) {
		// Nom obligatoire
		result.recordIfFailed(p.getName() != null && !p.getName().isBlank(), p, "Une place doit avoir un nom non vide");

		// Marquage ≥ 0
		result.recordIfFailed(p.getToken() >= 0, p, "Le marquage initial d'une place doit être ≥ 0");
	...
	public Boolean caseTransition(Transition t) {
		// Nom obligatoire
		result.recordIfFailed(t.getName() != null && !t.getName().isBlank(), t,
		...
		if (interval != null) {
			// Borne inférieure ≥ 0
			result.recordIfFailed(interval.getLowerBound() >= 0, t,
			...
			if (interval.isSetUpperBound()) {
				result.recordIfFailed(interval.getUpperBound() >= interval.getLowerBound(), t,
	...
	public Boolean caseArc(Arc arc) {
		// Poids > 0
		result.recordIfFailed(arc.getWeight() > 0, arc, "Le poids d’un arc doit être strictement positif");
		...
		boolean valid = (source instanceof Place && target instanceof Transition)
				|| (source instanceof Transition && target instanceof Place);

		result.recordIfFailed(valid, arc, "Un arc doit relier une place à une transition (ou l’inverse)");

		// Vérifie la cohérence avec l'attribut direction
		Direction declaredDirection = arc.getDirection();

		boolean directionIsCorrect = (declaredDirection == Direction.P2T && source instanceof Place
				&& target instanceof Transition)
				|| (declaredDirection == Direction.T2P && source instanceof Transition && target instanceof Place);

		System.out.println("Arc from: " + source.getClass().getSimpleName() + " to: "
				+ target.getClass().getSimpleName() + " direction: " + arc.getDirection());

		result.recordIfFailed(directionIsCorrect, arc,
