...
public final class Utils {
	 ...
	 * ensemble, alors on l'utilisera pour l'afficher (sinon on se contente de
	 * l'affichage par défaut offert par toString().
	 */
	private static final Set<String> NamingAttributes = Set.of("name", "nom", "id");

	/**
	 * "Devine" le nom d'un EObject en examinant ses attributs. Si l'objet a un
	 * attribut dont le nom fait partie des noms acceptés (de l'attribut static
	 * NamingAttributes) alors on utilise cette valeur, sinon on se contente de ce
	 * qui est retourné par toString().
	 * 
	 * Dans tous les cas, le nom affiché est le nom de l'object tel que deviné (ou
	 * rien) suivi du retour de toString(), qui donne des informations intéressantes
	 * pour le débogage.
	 * 
